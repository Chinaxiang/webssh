...
def make_handlers(loop, options):
    ...
    handlers = [
        (options.ctx if options.ctx != '' else '/', IndexHandler, dict(loop=loop, policy=policy,
                                  host_keys_settings=host_keys_settings)),
        ((options.ctx if options.ctx != '' else '') + '/ws', WsockHandler, dict(loop=loop))
    ]
    return handlers
...
def main():
    ...
    logging.info(
        'ContextPath {}'.format(options.ctx if options.ctx != '' else '/')
    )
    if ssl_ctx:
